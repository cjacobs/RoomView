...
- **M2 — USDZ import pipeline. ✅ Done.** `.fileImporter`, copy-into-storage (`ScanImporter`/`ScanLibrary`), `Scan` SwiftData record, async load (`Entity(contentsOf:)`) replaces the placeholder scene, scan persists across relaunch. Verified end-to-end on macOS directly and on iOS Simulator, against real captured room scans already on the user's Mac (`office.usdz`, `Kitchen.usdz`, `Garage.usdz` — a real garage scan already exists, useful for M4). Note: getting a file onto the iOS Simulator for testing requires Files app → Browse → On My iPhone as the drop target, or downloading via Safari from a local server — dragging a file directly onto the Simulator window tries to open it rather than depositing it into Files.
- **M3 — Clip plane, primary approach. ✅ Done.** `CustomMaterial` shader-discard, verified via a standalone `xcrun metal` compile of the shader before integrating (confirmed `discard_fragment()` works inside a RealityKit `[[visible]]` surface shader — the plan's flagged uncertainty). Two issues found via live testing against a real scan and fixed: base color texture wasn't applied by default (CustomMaterial's surface shader fully replaces shading, so the wrapped texture must be explicitly sampled), and the plane now tracks the camera's live view direction every frame (via a `SceneEvents.Update` subscription reading the orbit-controlled camera's transform) rather than a fixed world axis, so it stays perpendicular to wherever you're looking as you orbit.
- **M3.5 — Orbit/POV toggle with walkable avatar. ✅ Done.** Added on user request after seeing the clip plane still leave unwanted geometry in the way: a complementary camera mode, not a replacement for orbit. `SceneCameraMode` (`.orbit`/`.pov`) picked via a segmented control; `FirstPersonController` drives an avatar (position/yaw/pitch) and, while active, syncs a shared `PerspectiveCamera` to it each frame via the same `SceneEvents.Update` subscription pattern as the clip plane. Movement: WASD (macOS, held-key tracking via `.onKeyPress`) or an on-screen joystick (iOS); look: click/touch-drag, plus ijkl held-key turning on macOS. POV has a "zoomed out" slider (0 = true first-person) that pulls the camera back along the current view vector while keeping the same look direction, for seeing more of a room without leaving POV framing. `ClipPlaneController` gained an `isEnabled` flag, off while in POV so walking up to geometry doesn't slice it away. The avatar mesh itself (a simple cylinder+sphere+cone marker) is built but deliberately not added to the scene — visible when looking down, so rendering was disabled; the entity still exists for its transform. Two bugs found via live testing and fixed: switching `SceneContainerView`'s body between an `if/else` on camera mode was tearing down and recreating the RealityKit-backed view (visible as the clip-plane slider getting obscured by the scene) — fixed by unifying into one view with a `.gesture(_:isEnabled:)`-gated look gesture instead of branching; and returning to orbit mode left the camera at POV's last (possibly zoomed-out) transform with no orbit-mode control to fix it — fixed by having `FirstPersonController` save/restore the camera's transform itself exactly when it starts/stops driving the camera, in a `didSet` on `isDrivingCamera`, avoiding any dependency on cross-view SwiftUI update ordering.
- **M4 — Clip plane hardening + first real scan. In progress.** Started by surfacing a way to load a new scan without relaunching: a `+` button in the viewer HUD (both camera modes), reusing the same `ScanImporter`/`.fileImporter` flow as the first-run `ImportView` — since `RootView` already shows `scans.first` by `importedAt`, a fresh import just becomes the active scan automatically, no separate library view needed yet. Along the way: fixed a macOS system-beep-on-key-repeat bug (`.onKeyPress` was only listening for `.down`/`.up`, so auto-repeat's `.repeat` phase fell through unhandled), and added keyboard-driven ijkl orbit rotation to match the drag/pinch orbit controls — implemented in a new `OrbitCameraController` that recomputes yaw/pitch/radius fresh from the camera's live transform every tick (rather than tracking its own state) so it stays in sync with whatever the built-in drag gesture did to the camera between key presses. Still open: the second clip-plane orientation preset, and explicit validation of clip-plane cutting quality against `Garage.usdz`'s real dense mesh (imported and navigable now via the `+` button, but not yet specifically stress-tested). *Done when:* clip plane behaves acceptably on a real scan with both presets.
